...
# ── VOICE CONFIG ──────────────────────────────────────────────────────────────
# Unreal Speech voices (free tier: 250K chars/month — unrealspeech.com)
UNREALSPEECH_VOICES = {
    'short_hook':   'Will',   # energetic, punchy male
    'core_message': 'Will',   # confident, clear male
    'authority':    'Dan',    # deep, authoritative male
    'loop_video':   'Scarlett', # smooth, engaging female
}
...
VOICE_ENGINE = (
    'Unreal Speech Neural'
    if UNREALSPEECH_API_KEY else
...
def generate_voiceover(script, video_type, uid):
    ...
    if UNREALSPEECH_API_KEY:
        # ── Unreal Speech — free 250K chars/month ──────────────────────────
        voice = UNREALSPEECH_VOICES.get(video_type, 'Will')
        resp  = requests.post(
        ...
        logger.info(f'🎙️  Voice: Unreal Speech {voice}')
    else:
