import os, json, random, logging, threading, subprocess, requests, time, uuid
...
# Job tracker: {job_id: {status, progress, file, error}}
jobs = {}
...
def run_pipeline(job_id, video_type):
    ...
    try:
        ...
        jobs[job_id] = {'status':'done','step':'✅ Ready!','file':video_path,'error':None,'type':video_type,'caption':script_data.get('caption',''),'hashtags':script_data.get('hashtags','')}
        logger.info(f"✅ Job {job_id} done: {video_path}")
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn { border: none; border-radius: 14px; padding: 20px 12px; cursor: pointer; font-size: 14px; font-weight: 700; color: #fff; transition: all .2s; position: relative; overflow: hidden; }
  ...
  .b3 { background: linear-gradient(135deg, #F7971E, #FFD200); color: #000 !important; }
  .b4 { background: linear-gradient(135deg, #11998e, #38ef7d); color: #000 !important; }
  .b3 .label, .b3 .dur { color: #000; }
  .b4 .label, .b4 .dur { color: #000; }
  ...
  .card-title { font-size: 13px; color: #888; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
  ...
  .caption-box { background: #0d0d18; border-radius: 10px; padding: 14px; margin-top: 12px; font-size: 13px; color: #aaa; line-height: 1.6; }
  .hidden { display: none; }
  .error { color: #FF416C; font-size: 14px; margin-top: 10px; }
...
<div id="status-box" class="hidden">
  <div class="card">
    ...
    <div class="progress"><div class="progress-bar" id="prog" style="width:10%"></div></div>
  </div>
...
<script>
let pollTimer = null;
const steps = {'✍️ Writing script...': 20, '🎙️ Generating voiceover...': 50, '🎬 Building video...': 80, '✅ Ready!': 100};
...
function poll(jobId) {
  pollTimer = setInterval(() => {
    fetch('/status/' + jobId)
      .then(r => r.json())
      .then(data => {
        const pct = steps[data.step] || 10;
        document.getElementById('prog').style.width = pct + '%';

        if (data.status === 'running') {
          document.getElementById('step-text').innerHTML = '<span class="spinner"></span>' + data.step;
...
def generate():
    ...
    t = threading.Thread(target=run_pipeline, args=(job_id, video_type), daemon=True)
    t.start()
...
def status(job_id):
    job = jobs.get(job_id, {'status': 'not_found', 'step': '?', 'error': 'Job not found'})
    return jsonify(job)
...
def download(job_id):
    ...
    video_type = job.get('type', 'video')
    filename = f"trading_{video_type}_{job_id[:6]}.mp4"
