...
# ── VOICE CONFIG ──────────────────────────────────────────────────────────────
# Deepgram Aura voices ($200 free credit — deepgram.com)
DEEPGRAM_VOICES = {
    'short_hook':   'aura-orion-en',    # energetic, punchy male
    'core_message': 'aura-orion-en',    # confident, clear male
    'authority':    'aura-zeus-en',     # deep, commanding male
    'loop_video':   'aura-luna-en',     # smooth, hypnotic female
}
...
VOICE_ENGINE = (
    'Deepgram Aura Neural'
    if DEEPGRAM_API_KEY else
    'Microsoft Neural (free fallback)'
)
...
def generate_voiceover(script, video_type, uid):
    ...
    if DEEPGRAM_API_KEY:
        # ── Deepgram Aura — $200 free credit ───────────────────────────────
        # FIX: removed invalid &container=mp3 parameter (only none/wav are valid)
        ...
        logger.info(f'🎙️  Voice: Deepgram {voice}')
    else:
        # ── edge-tts fallback — free Microsoft Neural ───────────────────────
        # FIX: use new_event_loop() instead of asyncio.run() — safer in threads
        voice = EDGE_VOICES.get(video_type, 'en-US-AndrewNeural')
        loop = asyncio.new_event_loop()
        try:
            loop.run_until_complete(_tts_edge(script, voice, path))
        finally:
            loop.close()
